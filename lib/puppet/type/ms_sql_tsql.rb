...
Puppet::Type::newtype(:ms_sql_tsql) do
  ...
  desc 'command to run against an instance with the authenticated credentials used in ms_sql::config'
  newparam(:command) do

  end

  desc 'requires the usage of ms_sql::config with the user and password'
