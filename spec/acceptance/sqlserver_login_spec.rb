...
%w(2012 2014).each do |version|
  ...
  describe "sqlserver::login #{version}", :node => host do
    ...
    sqlserver::login{'<%= login_name %>':
      ...
      default_language  => '<%= defined?(default_language) ? default_language : 'us_english'%>',
    ...
    context 'delete a login' do
      ...
      it 'removes an existing login' do
        pending('This test is blocked by FM-2682')
        ensure_value = 'absent'
        pp = ERB.new(manifest).result(binding)
        name_query = "SELECT name FROM sys.server_principals WHERE name = '#{login_name}'"
        ...
        apply_manifest_on(host, pp) do |r|
          expect(r.stdout).not_to match(/Error/i)
          run_sql_query(host, {:query => name_query, :expected_row_count => 0}) do |result|
            expect(result.stdout).not_to match(/#{login_name}/)
          end
        end
      end
