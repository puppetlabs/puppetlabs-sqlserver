...
provider_class = Puppet::Type.type(:ms_sql_features).provider(:mssql)
...
RSpec.describe provider_class do
  ...
  shared_examples 'create' do |args, munged_args = {}|
    it {
      @resource = Puppet::Type::Ms_sql_features.new(args)
  ...
  shared_examples 'fail on' do |feature_params|
    it {
      expect {
        @resource = Puppet::Type::Ms_sql_features.new(feature_params)
        @provider = provider_class.new(@resource)
