  ...
  shared_examples 'mssql_tsql onlyif' do
    it {
      ...
      should_contain_onlyif.each do |check|
        should contain_ms_sql_tsql(mssql_tsql_title).with_onlyif(convert_to_regexp(check))
      end
    }
  ...
  shared_examples 'mssql_tsql command' do
    it {
      ...
      should_contain_command.each do |check|
        should contain_ms_sql_tsql(mssql_tsql_title).with_command(convert_to_regexp(check))
      end
    }
  ...
  shared_examples 'mssql_tsql without_command' do
    it {
      ...
      should_not_contain_command.each do |check|
        should_not contain_ms_sql_tsql(mssql_tsql_title).with_command(convert_to_regexp(check))
      end
    }
  end
